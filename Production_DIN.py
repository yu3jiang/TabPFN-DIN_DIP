...
MODEL_PATH = 'lgDIN.tabpfn_fit'
NC_DIR = r'./nCS_DINEOF'
OUTPUT_DIR = r'./nCS_DINEOF_OutPut_DIN'
...
def process_monthly_data(monthly_files, model):
    """Process monthly NC files, predict DIN in batches, and save results"""
    ...
    # Save results
    output_filename = f"DIN_prediction_{year}_{month:02d}.h5"
    output_path = os.path.join(OUTPUT_DIR, output_filename)

    try:
        with h5py.File(output_path, 'w') as hf:
            group = hf.create_group('DIN_Predictions')
            group.create_dataset('longitude', data=lon_1d, compression='gzip')
            group.create_dataset('latitude', data=lat_1d, compression='gzip')
            group.create_dataset('DIN', data=din_grid, compression='gzip')
            ...
            group.attrs['processing_note'] = 'Feature transformation: -SSS², ln(CHL), SST*cos(longitude/latitude); Results converted from log10(DIN) to actual DIN values'
...
def main():
    """Main function: Process NC files by month and generate DIN products"""
